...
import type { AllocationStrategy, BondGlidePath, GlidePathShape } from "@/types/calculator";
...
interface PersonalInfoSectionProps extends SectionProps {
  ...
  age1: number;
  setAge1: NumberSetter;
  age2: number;
  setAge2: NumberSetter;
  retirementAge: number;
  setRetirementAge: NumberSetter;
  isMar: boolean;
}
...
interface BalancesSectionProps extends SectionProps {
  taxableBalance: number;
  setTaxableBalance: NumberSetter;
...
interface ContributionsSectionProps extends SectionProps {
  isMar: boolean;
  age1: number;
  age2: number;
  cTax1: number;
  setCTax1: NumberSetter;
  cPre1: number;
  setCPre1: NumberSetter;
  cPost1: number;
  setCPost1: NumberSetter;
  cMatch1: number;
  setCMatch1: NumberSetter;
  cTax2: number;
  setCTax2: NumberSetter;
  cPre2: number;
  setCPre2: NumberSetter;
  cPost2: number;
  setCPost2: NumberSetter;
  cMatch2: number;
  setCMatch2: NumberSetter;
  person1TotalValidation: FieldValidationResult | null;
  person2TotalValidation: FieldValidationResult | null;
}

function ContributionLimitNotice({ validation }: { validation: FieldValidationResult | null }) {
...
export function ContributionsSection({
  ...
  cMatch2,
  setCMatch2,
  person1TotalValidation,
  person2TotalValidation,
  onInputChange,
}: ContributionsSectionProps) {
  const iraLimitTip1 = `2026 IRS limit: $${getMaxIRAContribution(age1).toLocaleString()}`;
  const iraLimitTip2 = `2026 IRS limit: $${getMaxIRAContribution(age2).toLocaleString()}`;
  ...
  return (
    <div className="grid grid-cols-1 gap-8">
      <div className="space-y-4">
        ...
        <Input label="Post-Tax (Roth) ($/yr)" value={cPost1} setter={setCPost1} step={500} onInputChange={onInputChange} defaultValue={getMaxIRAContribution(age1)} validate={(val) => validateIRAContribution(val, age1)} tip={iraLimitTip1} />
        ...
        <ContributionLimitNotice validation={person1TotalValidation} />
      </div>
      {isMar && (
        <div className="space-y-4">
          <Badge className="bg-blue-100 text-blue-800 hover:bg-blue-100 dark:bg-blue-900 dark:text-blue-100">
          ...
          <Input label="Post-Tax (Roth) ($/yr)" value={cPost2} setter={setCPost2} step={500} onInputChange={onInputChange} defaultValue={getMaxIRAContribution(age2)} validate={(val) => validateIRAContribution(val, age2)} tip={iraLimitTip2} />
          ...
          <ContributionLimitNotice validation={person2TotalValidation} />
        </div>
      )}
    </div>
  );
...
interface AssumptionsSectionProps extends SectionProps {
  selectId: string;
  returnMode: ReturnMode;
  setReturnMode: (value: ReturnMode) => void;
  setRandomWalkSeries: (value: WalkSeries) => void;
  retRate: number;
  setRetRate: NumberSetter;
  inflationRate: number;
  setInflationRate: NumberSetter;
  stateRate: number;
  setStateRate: NumberSetter;
  allocationStrategy: AllocationStrategy;
  setAllocationStrategy: (value: AllocationStrategy) => void;
  bondStartPct: number;
  setBondStartPct: NumberSetter;
  bondEndPct: number;
  setBondEndPct: NumberSetter;
  bondStartAge: number;
  setBondStartAge: NumberSetter;
  bondEndAge: number;
  setBondEndAge: NumberSetter;
  glidePathShape: GlidePathShape;
  setGlidePathShape: (value: GlidePathShape) => void;
  bondGlidePath: BondGlidePath | null;
  age1: number;
  wdRate: number;
  setWdRate: NumberSetter;
  incRate: number;
  setIncRate: NumberSetter;
  incContrib: boolean;
  setIncContrib: BooleanSetter;
}
...
interface AssetAllocationControlsProps extends SectionProps {
  selectId: string;
  allocationStrategy: AllocationStrategy;
  setAllocationStrategy: (value: AllocationStrategy) => void;
  bondStartPct: number;
...
function AssetAllocationControls({
  selectId,
  allocationStrategy,
  setAllocationStrategy,
  bondStartPct,
        ...
        <div className="space-y-4 p-4 bg-blue-50 dark:bg-blue-950/30 rounded-lg border border-blue-200 dark:border-blue-800">
          ...
          <div className="space-y-2">
            ...
            <select
              ...
              onChange={(e) => {
                setGlidePathShape(e.target.value as GlidePathShape);
                onInputChange();
              }}
              className="flex h-11 md:h-10 w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm shadow-sm ring-offset-white transition-colors file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-gray-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 dark:border-gray-600 dark:bg-gray-800"
            >
...
interface AdvancedSettingsSectionProps extends SectionProps {
  selectId: string;
  isMar: boolean;
  includeSS: boolean;
  setIncludeSS: BooleanSetter;
  ssIncome: number;
  setSSIncome: NumberSetter;
  ssClaimAge: number;
  setSSClaimAge: NumberSetter;
  ssIncome2: number;
  setSSIncome2: NumberSetter;
  ssClaimAge2: number;
  setSSClaimAge2: NumberSetter;
  includeMedicare: boolean;
  setIncludeMedicare: BooleanSetter;
  medicarePremium: number;
  setMedicarePremium: NumberSetter;
  medicalInflation: number;
  setMedicalInflation: NumberSetter;
  includeLTC: boolean;
  setIncludeLTC: BooleanSetter;
  ltcAnnualCost: number;
  setLtcAnnualCost: NumberSetter;
  ltcProbability: number;
  setLtcProbability: NumberSetter;
  ltcDuration: number;
  setLtcDuration: NumberSetter;
  ltcOnsetAge: number;
  setLtcOnsetAge: NumberSetter;
  ltcAgeRangeStart: number;
  setLtcAgeRangeStart: NumberSetter;
  ltcAgeRangeEnd: number;
  setLtcAgeRangeEnd: NumberSetter;
  enableRothConversions: boolean;
  setEnableRothConversions: BooleanSetter;
  targetConversionBracket: number;
  setTargetConversionBracket: NumberSetter;
}
...
interface SocialSecuritySettingsProps extends SectionProps {
  isMar: boolean;
  includeSS: boolean;
  setIncludeSS: BooleanSetter;
  ssIncome: number;
...
function SocialSecuritySettings({
  isMar,
  includeSS,
  setIncludeSS,
  ssIncome,
...
interface MedicareSettingsProps extends SectionProps {
  includeMedicare: boolean;
  setIncludeMedicare: BooleanSetter;
  medicarePremium: number;
  setMedicarePremium: NumberSetter;
...
interface LongTermCareSettingsProps extends SectionProps {
  includeLTC: boolean;
  setIncludeLTC: BooleanSetter;
  ltcAnnualCost: number;
  setLtcAnnualCost: NumberSetter;
...
interface RothConversionSettingsProps extends SectionProps {
  selectId: string;
  enableRothConversions: boolean;
  setEnableRothConversions: BooleanSetter;
  targetConversionBracket: number;
...
function RothConversionSettings({
  selectId,
  enableRothConversions,
  setEnableRothConversions,
  targetConversionBracket,
